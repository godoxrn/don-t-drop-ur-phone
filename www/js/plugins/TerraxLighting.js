...
//=============================================================================
 /*:
 * @plugindesc v1.2.1 Creates an extra layer that darkens a map and adds lightsources!
 * @author Terrax
 *
 * @param Player radius
...
*Light 200 #ffffff B50  增加光亮度50%。值在0到99之间。

*Light 200 # ffffff D1  将半个lightglobe放在墙里。
...
*要改变游戏中的玩家半径，使用以下插件命令：

Light radius 200 #FFFFFF（改变半径和颜色）

如果你想慢慢改变玩家的半径（就像一个垂死的火炬）
...
给正常的光源定义： Light 250 #FFFFFF 和一个号码

变成 'Light 250 #FFFFFF 1'
（如果你使用可选的亮度和方向放在后面 (Light 250 #FFFFFF B50 D2 1)


打开此灯使用插件命令： 'Light on 1'.

插件的命令也触发了目标光源的SelfSwitch的“D”。
要关闭光使用插件命令： 'Light off 1'.


您可以重置开关每个地图的选项或手工，使用插件命令 'Light switch reset' 

用'fire'替换'light' 会产生一个轻微的闪烁光。
你可以用插件命令 'SetFire 7 10' 
其中7是半径变化，10是颜色由红色变为黄色。



完全关闭脚本使用：'Light deactivate' 
要再次启用它，请使用命令：'Light activate'



激活地图上的昼夜周期，放在一个触发的事件上注明 'DayNight' 
或在地图上注明。

插件命令'Daynight speed 10' 改变速度。

速度10意味着在游戏中每10秒1小时（可能太快了）。

插件命令'Daynight hour 16 30' 设置
时间到16：30
每小时都有自己的色彩值。

插件命令 'Daynight color 0 #222222' 设置0：00的颜色值为#222222



如果你想利用每天的时间来触发效果（比如天黑时开灯）
...
如果你设置它的值，5为例，它将在变量5中存储值。
然后您可以使用该变量触发灯光。


帮助同步/调试的时间系统可以使用'daynight debug' 显示当前时间。


如果你想去“外星世界”或什么，你可以时间，用脚本命令 'daynight hoursinday 48' （48小时一天，别忘了填写的小时值）



作为一种替代的日夜周期可以使用色彩系统。如果你想使用另一个插件
来做白天/夜晚循环，最好使用。


插件命令的 'Tint set #333333' 将使房间轻度黑暗。


插件命令的'Tint fade #777777 5'会褪色的颜色从当前色新，
最后
数字（5）是淡入淡出的速度，1是快速淡入淡出，20是很慢的淡入淡出。


如果一个地区有一个日夜循环系统、色彩系统被禁用。



使用手电筒效果脚本命令  'Flashlight on 8 12 #FFFFFF 3' 和  'Flashlight off'  

参数是可选的 (8=beamlength, 12=beamwidth, #FFFFFF=color, 3=beam density)


事件也可以使用手电筒效果。在注释标签用 'Flashlight 8 12 #888888 1 2'。

其中8是手电筒光束的长度，12是光束的宽度。最后的数字是
可选，可用于打开或关闭NPC的手电筒，并设置光束的方向。
如果事件不移动（1 =上，2 =右，3 =下，4 =左）默认是向下。
...
TileLight 1 ON #FFFFFF 150将在所有有tile-tag1标记的瓷砖上创建一个光源（彩色# ffffff半径150）。

TileRegion 1 ON #FFFFFF 150 将在所有region1上创建一个光源。


你可以选择增加一个光源的亮度，用 TileRegion 1 ON #FFFFFF 150 B50 （50%增加亮度）
TileLight 1 OFF 将关闭 tile-tag 1 上的光
TileRegion 1 OFF 同上
TileFire and RegionFire同上
确保你的地图上至少有一个有灯光的事件，否则脚本不会运行。


TileBlock和regionblock设置

要阻止某些瓷砖（例如屋顶）上的灯光，可以使用以下插件命令。
1 tileblock # 000000块光瓦瓦标签1。
1 regionblock # 000000块灯瓦区1号。
tileblock 1和tileregion 1关闭关闭阻塞了。
变暗但不完全阻止光使用稍高的颜色设置（# 333333）例如。
这个函数不使用光线追踪。如果玩家lightradius大于砖堵
光线会在另一面显现。为了达到最好的效果保持lightradius小一点的那块段。
