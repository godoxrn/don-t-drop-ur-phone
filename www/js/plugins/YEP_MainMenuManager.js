 ...
 * ============================================================================
 *
 * 这可以让你对游戏主菜单直接控制，集合了添加，移除，隐藏，显示，开启，关闭
 * 等命令
 * 对于希望改变主菜单各个方面内容的人来说，不需要接触源代码，使用这个插件就
 * 可以做到。尽管这个插件在插件参数里面设置了很多菜单生成的方案，但它依然考
 * 虑了采用一个简洁的方式来处理菜单命令。
 *
 * ============================================================================
 ...
 * ============================================================================
 *
 * 参数里被分成不同的部分。每个部分都发挥了菜单的不同功能。下面是每个部分的
 * 定义:
 *
 * Name
 * - 名字-这是在主菜单实际显示的内容。这是一个函数，这意味着，是代码驱动的。
 * 所以如果你想要菜单就是显示你输入的文本，请使用引号('')
 *
 * Symbol
 * - 符号-这是命令的ID。每个命令应该有独有的符号，这样不同命令之间就不会发生
 * 冲突。只要你明白被选择的菜单发挥相同的功能，共享符号也是可以的。
 *
 * Show
 * - 显示-这是一个关于菜单是否显示的函数状态。如果你希望它显示出来，可以使用
 * 没有引号的true
 *
 * Enabled
 * - 开启-这是一个关于菜单是否开启的函数状态。这和显示命令不同的地方在于，如
 * 果菜单不是开启状态，那么即便显示出来也无法选择。如果你希望它开启，可以使
 * 用没有引号的true
 * 
 * Ext
 * - 拓展-这代表拓展功能。这个服务是作为辅助符号使用的，他可以用来做很多事情
 * 。除非你想要命令和某个拓展值联系起来，否则他没有直接的影响。大多数命令不
 * 需要使用拓展值
 *
 * Main Bind
 * - 主要绑定-这是函数式，当菜单被选择时，它就会运行。这个功能主要是用来绑定
 * 命令，这就允许你可以用各种方式来打开视图。对于命令来说，你可以先选择一个
 * 角色，然后用this.commandItem.bind(this)这段命令。
 *
 * Actor Bind
 * -角色绑定-这是函数式。当你选择一名角色再来选择菜单时，他就会运行，通常用
 * 来展示。当不需要特定角色展示时，这个功能可以不使用。
 *
 * ============================================================================
 * Examples
 * ============================================================================
 ...
 * Actor Bind:
 *
 * 物品菜单就是用这命令制作的。TextManager.item就是绑定名字，这个名字来源于
 * 数据库的Item选项。符号item就是用来做这个菜单的特殊ID。为了让菜单显示出来
 * ，这里用needsCommand函数来检查，这个函数将和数据库设置的item选项关联，来
 * 决定是否显示。为确保菜单开启，我们将会检查主菜单是否开启，这和初始队伍里
 * 有无角色有关。最后，this.commandItem.bind(this)是用来开启物品菜单的。
 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *
 ...
 *
 * 技能菜单就是用这命令制作的。TextManager.skill就是绑定名字，这个名字来源
 * 于数据库的skill选项。符号skill就是用来做这个菜单的特殊ID。为了让菜单显示
 * 出来，这里用needsCommand函数来检查，这个函数将和数据库设置的item选项关联
 * ，来决定是否显示。为确保菜单开启，我们将会检查主菜单是否开启，这和初始队
 * 伍里有无角色有关。这次，我们的主要绑定命令会用this.commandPersonal.bind
 * (this)来替代。一旦玩家选择了一名角色，SceneManager.push(Scene_Skill)将会
 * 运行来打开角色技能管理。
 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *
 ...
 * Actor Bind:
 *
 * 这是一个插件默认包含的定制菜单命令。命令名字为公共事件1，但是你可以通过
 * 参数设置引号内的文本来自定义。这个符号就是对于所有的公共事件。然而，默认
 * 的公共事件选项是隐藏的，因此如果你需要，可以设置显示选项为true。这个命令
 * 默认是开启可以被选择的。拓展值实际上发挥了很大的作业，它意味着哪个公共事
 * 件被执行。在这里例子，拓展值是1，所以公共事件1将会在菜单被选中时执行，如
 * 果设置为25，则公共事件25会被执行。其原理是主要绑定的命令为
 * this.callCommonEvent.bind(this)。这是一个允许公共事件执行的函数。
 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *
 * ============================================================================
